module KineticSdk
  class RequestCe
    ...
    # Create a Space security policy definition
    #
    ...
    def create_space_security_policy_definition(body, headers=default_headers)
      puts "Adding Space Security Policy Definition \"#{body['name']}\"."
    ...
    def delete_space_security_policy_definition(name, headers=default_headers)
      puts "Deleting Space Security Policy Definition \"#{name}\"."
      # Delete the space security policy definition
      delete("#{@api_url}/securityPolicyDefinitions/#{url_encode(name)}", headers)
    end
    ...
    def delete_space_security_policy_definitions(headers=default_headers)
      JSON.parse(find_space_security_policy_definitions({}, headers))["securityPolicyDefinitions"].each do |s|
    ...
    def find_space_security_policy_definitions(params={}, headers=default_headers)
      puts "Listing Space Security Policy Definitions."
      get("#{@api_url}/securityPolicyDefinitions", params, headers)
    end

    # Retrieve a single Space security policy definition
    #
    # @param name [String] name of the security policy definition
    # @param params [Hash] Query parameters that are added to the URL, such as +include+
    # @param headers [Hash] hash of headers to send, default is basic authentication and JSON content type
    # @return [RestClient::Response] Response object, with +code+ and +body+ properties
    def retrieve_space_security_policy_definition(name, params={}, headers=default_headers)
      puts "Retrieving Space Security Policy Definition \"#{name}\""
      get("#{@api_url}/securityPolicyDefinitions/#{url_encode(name)}", params, headers)
    end
    ...
    def update_space_security_policy_definition(name, body, headers=default_headers)
      puts "Updating Space Security Policy Definition \"#{name}\""
      put("#{@api_url}/securityPolicyDefinitions/#{url_encode(name)}", body, headers)
    end



    # Create a Kapp security policy definition
    #
    ...
    # @param headers [Hash] hash of headers to send, default is basic authentication and JSON content type
    # @return [RestClient::Response] Response object, with +code+ and +body+ properties
    def create_security_policy_definition(kapp_slug, body, headers=default_headers)
      puts "Adding Security Policy Definition \"#{body['name']}\" to the \"#{kapp_slug}\" kapp."
    ...
    def delete_security_policy_definition(kapp_slug, name, headers=default_headers)
      puts "Deleting Security Policy Definition \"#{name}\" from the \"#{kapp_slug}\" kapp."
      # Delete the kapp security policy definition
      delete("#{@api_url}/kapps/#{kapp_slug}/securityPolicyDefinitions/#{url_encode(name)}", headers)
    end
    ...
    def delete_security_policy_definitions(kapp_slug, headers=default_headers)
      JSON.parse(find_security_policy_definitions(kapp_slug, {}, headers))["securityPolicyDefinitions"].each do |s|
    ...
    def find_security_policy_definitions(kapp_slug, params={}, headers=default_headers)
      puts "Listing Security Policy Definitions on the \"#{kapp_slug}\" kapp."
      get("#{@api_url}/kapps/#{kapp_slug}/securityPolicyDefinitions", params, headers)
    end

    # Retrieve a single Kapp security policy definition
    #
    # @param kapp_slug [String] slug of the kapp
    # @param name [String] name of the security policy definition
    # @param params [Hash] Query parameters that are added to the URL, such as +include+
    # @param headers [Hash] hash of headers to send, default is basic authentication and JSON content type
    # @return [RestClient::Response] Response object, with +code+ and +body+ properties
    def retrieve_security_policy_definition(kapp_slug, name, params={}, headers=default_headers)
      puts "Retrieving Security Policy Definition \"#{name}\" on the \"#{kapp_slug}\" kapp."
      get("#{@api_url}/kapps/#{kapp_slug}/securityPolicyDefinitions/#{url_encode(name)}", params, headers)
    end
    ...
    def update_security_policy_definition(kapp_slug, name, body, headers=default_headers)
      puts "Updating Security Policy Definition \"#{name}\" on the \"#{kapp_slug}\" kapp."
      put("#{@api_url}/kapps/#{kapp_slug}/securityPolicyDefinitions/#{url_encode(name)}", body, headers)
    end
