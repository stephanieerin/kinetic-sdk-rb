...
module KineticSdk
  class Task
    ...
    def export_handler(definition_id, headers=header_basic_auth)
      ...
      puts "Exporting handler \"#{definition_id}\" to #{@options[:export_directory]}."
      ...
      puts "Exported handler: #{definition_id} to #{handler_file}"
    end
